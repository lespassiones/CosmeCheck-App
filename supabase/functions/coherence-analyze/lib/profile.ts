/**
 * Best-effort prompt enrichment — compact Deno port of
 * CosmetWiki/lib/skin/promptFormat.ts + lib/restrictions/promptFormat.ts.
 *
 * Reads cosme_check.user_profiles.preferences (jsonb) for the signed-in user
 * and renders the same two prompt blocks the web splices into the coherence
 * conclusion. Fails closed (returns null) on any error or empty profile so a
 * missing/partial profile never breaks the pipeline — exactly like the web,
 * which treats this as optional personalisation.
 *
 * Note: this mirrors the web's prompt format and field names. It does NOT
 * replicate the full legacy-value migration / label maps from the web's
 * readSkinProfile (those are large, UI-coupled, and only affect cosmetic
 * wording). Concern/type values are rendered as stored.
 */
...
/**
 * One round-trip: read preferences once + the ingredient_families label map,
 * return both prompt blocks. Best-effort: returns {null, null} on any error.
 */
export async function loadProfileAndRestrictions(
  ...
  try {
    ...
    // Only fetch the families label map if restrictions reference families.
