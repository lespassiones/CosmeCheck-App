// Reduces an HTML page to a token-cheap excerpt focused on the INCI section,
// then asks GPT-4o-mini (primary) to EXTRACT (not generate) the ingredient
// list. Mistral kept as fallback. Port of CosmetWiki
// lib/productSearch/extractWithMistral.ts, reusing the shared aiClient.
import { AI_MODEL, hasMistral, hasOpenAI, mistralChat, openai } from "../../_shared/aiClient.ts";
...
function reduceHtmlForExtraction(html: string): string {
  ...
  // Step 1: locate an INCI-shaped sequence (AQUA/WATER followed by uppercase
  // tokens). Most reliable signal on pharma sites where "composition" keywords
  // land on CSS class names far upstream of the real list.
  const inciShape =
  ...
  // Step 2: keyword-driven excerpts. Up to 3 hits, dedupe their windows.
...
export async function extractInciFromHtml(input: {
  ...
  const prompt =
    `Tu es un extracteur d'INCI. Voici le texte brut d'une page produit cosmétique. Trouve UNIQUEMENT la liste INCI (la liste des ingrédients) telle qu'elle apparaît, et renvoie-la en une seule ligne, ingrédients séparés par des virgules.
...
"""`;
  ...
  // Primary: GPT-4o-mini. Hard 12 s timeout to stay within serverless budget.
  if (hasOpenAI()) {
    try {
  ...
  // Fallback: Mistral. Useful if OpenAI is down or the key is missing.
