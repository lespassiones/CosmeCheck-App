...
| **4** | ~~Toi~~ | ✅ **Fiche créée le 20/08/2026** : « Cosme Check: scan cosmétique » (28 caractères), français, UGS `cosmecheck-ios`, accès complet. Reste à poser la catégorie principale **Style de vie**. ⚠️ **Pas Médecine, pas Santé et forme** : la catégorie médicale déclenche une revue bien plus dure sur les affirmations produit | tout ce qui suit, et l'`ascAppId` dont EAS a besoin |
| **5** | ~~Toi~~ | ✅ **Faite le 20/08/2026**, rôle Admin, `ASC_API_KEY_ID = V8UJ847B48`, `ISSUER_ID = 7ff0f8c5-16e0-4ba8-8dad-76b2a629c4e3`. **Éprouvée en appelant vraiment l'API**, qui rend la fiche `com.cosmecheck.app` (id `6803527040`), et non en lisant la console. ⚠️ Le `.p8` est à la racine du dépôt, **ignoré par git** (`.gitignore:70`), et sauvegardé dans `D:\MesApps\Origma\CosmeCheck` : il ne se retéléchargera jamais. Ancienne consigne, gardée pour mémoire : C'est l'équivalent Apple du compte de service Google Play : elle remplace ton mot de passe et laisse EAS créer les certificats, les profils, et déposer les binaires sans intervention. ⚠️ Rôle Admin et non App Manager : c'est lui qui autorise la gestion complète des certificats, et un rôle trop étroit échoue **au milieu** d'un build avec un message qui ne parle pas de droits | tous les builds iOS, sans exception |
...
| **11** | ~~Claude~~ | ✅ **Fait le 20/08/2026.** `appl_Nsylpaup…` dans le `.env` (gitignoré) **et** créée chez EAS (portée projet, `plaintext`, environnement `production`), relue depuis le serveur. `eas.json` déclare maintenant `"environment": "production"` sur le profil de production, sinon rien ne garantit qu'EAS charge ces variables. ⚠️ **Vérifié au passage, et c'était le vrai risque** : EAS construit depuis l'archive **git**, donc `.env` n'y monte jamais. Les trois variables Supabase y étaient déjà, sans quoi le binaire aurait planté au lancement sur `createClient`. ⚠️ **La clé Android n'est PAS chez EAS, et c'est voulu** : Expo insère dans le bundle toutes les `EXPO_PUBLIC_*` définies, donc l'ajouter mettrait la caisse Android dans le binaire iOS. Chaque binaire ne porte que la clé de sa caisse | un binaire qui sait encaisser |
| **12** | ~~Claude~~ | ✅ **Fait le 20/08/2026** : `submit.production.ios` porte `ascAppId 6803527040`, `appleTeamId 7F96ZVZ4PC`, et les trois champs de la clé d'API, donc `eas submit` ne posera aucune question. ⚠️ Le chemin `./AuthKey_V8UJ847B48.p8` désigne un fichier **ignoré par git** : sur un poste neuf, il faut le reposer depuis la sauvegarde ou avoir chargé la clé chez EAS (`eas credentials`). Consigne d'origine : **`eas.json`** : ajouter le profil d'envoi iOS (`appleId`, `ascAppId`, `appleTeamId` = `7F96ZVZ4PC`) et le bloc `ios` du profil `production`. Aujourd'hui `submit.production` ne connaît qu'Android, et `track: "internal"` + `releaseStatus: "draft"` ne correspondent plus à la réalité de production | `eas submit -p ios` |
...
| **15** | ~~Toi~~ | ✅ **Build 1 réussi le 21/08/2026 à 00h39**, id `024135e8`, en **6 min 41 s**, commit `f561343`. ⚠️ **Le premier build DOIT être interactif** : EAS refuse de valider un certificat de distribution en non interactif (« Distribution Certificate is not validated for non-interactive builds »). Une fois le certificat accepté, les suivants passent en non interactif, moi compris. Consigne d'origine : **Le premier build iOS**, en interactif une seule fois, pour accepter la création du certificat de distribution : `eas build -p ios --profile production`. ⚠️ Aucun Mac n'est nécessaire, EAS construit dans le nuage. Le seul coût est la file d'attente du plan gratuit, mesurée à plus de quinze minutes sur RevealChat | tous les builds iOS suivants, non interactifs |
| **16** | ~~Claude~~ | ✅ **Fait le 21/08/2026 sur l'`.ipa` téléchargé**, avant tout envoi. Les six secrets serveur cherchés **par leur valeur exacte** tirée du `.env` : aucun dans le bundle. Clé RevenueCat iOS présente, clé Android absente, aucun appel direct à `api.mistral.ai`, `api.openai.com` ni `api.anthropic.com`. Habilitations du profil embarqué : `aps-environment = production`, `com.apple.developer.applesignin = Default`, `get-task-allow = false`. ⚠️ **Un motif `sk-` a été trouvé et c'était un faux positif** : `webkit-mask-positionRun`, un nom de propriété CSS dans la table de chaînes de Hermes. Avec une frontière de mot, zéro occurrence. Un contrôle qui crie au loup apprend à être ignoré. ⚠️ **Ma première version cherchait `aps-environment` dans l'`Info.plist`** : les habilitations vivent dans `embedded.mobileprovision`. Consigne d'origine : **Audit de secrets sur l'`.ipa` réellement livré** : aucune clé Mistral, OpenAI, `service_role`, Brevo ou Sentry dans le bundle, aucun appel direct à `api.mistral.ai` ni `api.openai.com`, et la clé RevenueCat **iOS** présente. ⚠️ Le nom du bundle change de plateforme : `index.android.bundle` devient `main.jsbundle`. Un contrôle qui échoue pour la mauvaise raison apprend à passer outre | la confiance dans ce qu'on publie |
| **16b** | ~~Toi~~ | ✅ **Clé APNs rattachée le 21/08/2026** : `PPD7WGA986`, téléversée en mode manuel (« Add a new push key » puis **non** à la génération), donc sans mot de passe Apple et sans entamer le second des deux emplacements. ⚠️ EAS le dit lui-même, « Unable to validate push key, you are not authenticated with Apple » : la clé n'a **pas** été validée auprès d'Apple. Une erreur de fichier serait **totalement silencieuse**, l'app obtiendrait la permission de notifier et rien n'arriverait jamais. Le seul contrôle qui vaille est de recevoir une vraie notification sur l'iPhone |
| **16c** | ~~Apple~~ | ✅ **Binaire déposé et traité le 21/08/2026** : build 1, `processingState = VALID`, `internalBuildState = IN_BETA_TESTING`. Le groupe « Interne » y a accès sans geste supplémentaire |
...
| **21** | Claude | **Captures iPhone 1 284 × 2 778** (écran 6,5 pouces, portrait, jusqu'à 10, et **seules les 3 premières** apparaissent sur les fiches d'installation). ⚠️ **Corrigé le 20/08/2026 sur ce que la console réclame vraiment** : elle accepte 1 242 × 2 688 ou 1 284 × 2 778, pas le 6,9 pouces que j'avais écrit. C'est exactement le format déjà produit pour RevealChat. ⚠️ Les 8 visuels de la fiche Play sont au format d'un écran Android et Apple exige des **dimensions exactes** : il faut les remettre à l'échelle sans déformation, puis ajouter une bande en haut et en bas, **chacune dans la couleur de son propre bord** échantillonnée sur quelques pixels, ce qui les rend invisibles. Les 8 originaux sont récupérables par l'API Play, donc rien à refaire à la main. Aucune capture iPad, `supportsTablet: false` |
...
# Lire l'état des deux magasins sans rien changer
node scripts/play-etat.mjs                          # pistes, abonnements, fiche, visuels
node scripts/asc-etat.mjs                           # fiche, versions, builds, abonnements iOS
```
