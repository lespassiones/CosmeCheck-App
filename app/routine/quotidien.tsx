/**
 * QuotidienScreen — page « Produits du quotidien » (kind = 'staple').
 *
 * Les staples (déo, dentifrice, gel douche, parfum, vernis...) ne suivent PAS
 * la logique matin/soir ni l'ordre : ils s'accumulent en simple liste avec une
 * fréquence. On peut les déplacer vers « Ma routine soin » (icône étoile) si
 * l'utilisateur veut les organiser comme un soin.
 *
 * Volontairement sans drag, sans sections, sans suggestions IA : c'est une
 * liste d'inventaire, pas une routine ordonnée.
 */
...
const QuotidienScreen: FC = () => {
  ...
  const staples = useMemo(
    () => items.filter((it) => it.kind === 'staple' && it.analysis),
    [items],
  )

  const deck = useAlternativesDeck(staples)
  ...
  const handleAddFromHistory = useCallback(
    async (analysisId: string, kind: RoutineItem['kind']) => {
      try {
        ...
        Alert.alert('Erreur', "Impossible d'ajouter ce produit.")
      }
    },
    [addToRoutine],
  )
    ...
    <View style={styles.root}>
      ...
      <SafeAreaView style={styles.safe} edges={['top']}>
        <View style={styles.topBar}>
          ...
          <Text style={styles.topTitle}>Produits du quotidien</Text>
        ...
        {isLoading ? (
          ...
          >
            <Pressable
              ...
              accessibilityRole="button"
              accessibilityLabel="Ajouter un produit du quotidien"
            >
            ...
            {staples.length === 0 ? (
              <View style={styles.emptyWrap}>
                <Ionicons name="cart-outline" size={40} color={colors.inkLight} />
                <Text style={styles.emptyText}>
                  Aucun produit du quotidien. Ajoute tes essentiels (déo, dentifrice, gel douche...)
